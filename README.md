...
1. Use Supabase project `fnkkeadpkjshsnjmoznl` and run every SQL file in `supabase/migrations` in filename order. Existing installations must run `202607150001_inventory_reporting.sql`, `202607160001_document_snapshots_and_issuers.sql`, `202607160002_document_methods.sql`, `202607160003_editable_item_descriptions.sql`, `202607160004_editable_item_brands.sql`, `202607160005_delivery_order_invoice_link.sql`, `202607160006_save_invoice_only.sql`, `202607160007_invoice_partial_deliveries.sql`, `202607170001_delivery_contacts.sql`, `202607170002_invoice_title.sql`, `202607170003_delivery_order_linked_items.sql`, `202607170004_product_parent_skus.sql`, `202607200001_linked_stock_products.sql`, `202607200002_invoice_hard_delete.sql`, then `202607200003_parent_sku_delivery_validation.sql` before deploying this version.
...
Invoices support multiple linked Delivery Orders with item-level partial-delivery controls. Linked Delivery Orders are removed only through their parent Invoice so all active inventory movements are reversed atomically. The User Management and Company Settings interfaces are intentionally not exposed; authentication accounts and centralized company document details remain unchanged.
